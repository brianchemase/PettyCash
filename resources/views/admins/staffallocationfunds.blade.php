...
<main class="content">
				<div class="container-fluid p-0">
					...
					<h1 class="h3 mb-3">Staff Fund Allocation </h1>
									...
									<form class="row row-cols-md-auto align-items-center" action="{{route('fundallocations')}}" method="GET" autocomplete="off">
										...
										<div class="col-12">
											...
											<div class="input-group mb-2 mr-sm-6">
												<div class="input-group-text">Get Staff</div>
												<select class="form-select" id="validationDefault04" name="q" required>
													<option selected disabled value="">Choose...</option>
													@forelse ($clients_list as $data)
													<option value="{{ $data->staff_id }}">{{ $data->first_name }}  {{ $data->last_name }}- {{ $data->staff_id }}</option>
													@empty
													<option value="" disabled>No Active Staff</option>
													@endforelse
												</select>
								...
								<div class="card-body">
									<form class="row g-3" method="POST" action="{{ route('allocate.funds') }}">
										...
										<div class="col-md-4">
                                            ...
                                            <input type="hidden" class="form-control" id="validationDefault01" name="staffid" value="{{$staff_id}}">
										</div>
										<div class="col-md-4">
										...
										<div class="col-md-4">
											<label for="validationDefaultUsername" class="form-label">Staff ID</label>
											<div class="input-group">
												<span class="input-group-text" id="inputGroupPrepend2">@</span>
												<input type="text" class="form-control" id="validationDefaultUsername" aria-describedby="inputGroupPrepend2" value="{{$staff_id}}" required>
											</div>
										</div>
